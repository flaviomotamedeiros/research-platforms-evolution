...
function build(): Container {
  ...
  const jwt = new JwtService({
    secret: process.env.GLPI_JWT_SECRET ?? 'insecure-dev-secret',
    accessTtl: '30m',
    refreshTtl: '7d',
  })
...
const globalForContainer = globalThis as unknown as { container_glpi?: Container }

export function getContainer(): Container {
  globalForContainer.container_glpi ??= build()
  return globalForContainer.container_glpi
}
