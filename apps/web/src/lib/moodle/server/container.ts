...
function build(): Container {
  ...
  const jwt = new JwtService({
    secret: process.env.MOODLE_JWT_SECRET ?? 'insecure-dev-secret',
    accessTtl: '30m',
    refreshTtl: '7d',
  })
...
const globalForContainer = globalThis as unknown as { container_moodle?: Container }

export function getContainer(): Container {
  globalForContainer.container_moodle ??= build()
  return globalForContainer.container_moodle
}
