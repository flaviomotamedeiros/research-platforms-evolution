...
import { StatTile, Bar, Skeleton } from '@/components/glpi/ui'
import { StatusBadge, PriorityBadge } from '@/components/glpi/glpi-ui'
              ...
              {r.byTechnician.map((t) => (
                <li key={t.technicianId} className="grid grid-cols-[160px_1fr_auto] items-center gap-3">
                  ...
                  <div className="flex h-3 overflow-hidden rounded-full bg-track">
                    <div className="h-full bg-gbrand" style={{ width: `${(t.open / maxTech) * 100}%` }} />
                    <div className="h-full bg-teal" style={{ width: `${(t.solved / maxTech) * 100}%` }} />
                  </div>
                  <span className="text-right text-xs tabular-nums text-ink-2">
                    <span className="font-bold text-gbrand-dark">{t.open} open</span> · {t.solved} solved
                  </span>
                </li>
              ))}
            </ul>
            <div className="mt-4 flex gap-4 text-xs text-ink-3">
              <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-gbrand" /> Open</span>
