        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
              filter === f.key ? 'bg-gbrand text-white' : 'text-ink-2 hover:text-ink-1'
            }`}
          >
            {f.label}
          </button>
      ...
      <div className="overflow-hidden rounded-2xl bg-surface-1 shadow-card">
        {tickets ? (
          <ul className="divide-y divide-black/5">
            {shown.map((t) => (
              <li key={t.id}>
                <Link href={`/glpi/tickets/${t.id}`} className="flex flex-wrap items-center gap-x-4 gap-y-2 px-6 py-4 transition-colors hover:bg-surface-0">
