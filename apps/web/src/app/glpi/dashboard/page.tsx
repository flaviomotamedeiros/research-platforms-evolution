...
import { StatTile, Skeleton } from '@/components/glpi/ui'
  ...
  return (
    <AppShell>
      <div className="mb-8 flex items-end justify-between">
        ...
        {me?.role === 'requester' && (
          <Link href="/glpi/tickets" className="rounded-xl bg-gbrand px-4 py-2 text-sm font-semibold text-white hover:bg-gbrand-dark">
            My tickets
          </Link>
        )}
      </div>
      ...
      <div className="mb-3 flex items-baseline justify-between">
        ...
        <Link href="/glpi/tickets" className="text-sm font-medium text-gbrand hover:underline">View all →</Link>
      </div>
        ...
        {tickets ? (
          <ul className="divide-y divide-black/5">
            {tickets.slice(0, 6).map((t) => (
              <li key={t.id}>
                <Link href={`/glpi/tickets/${t.id}`} className="flex items-center gap-4 px-6 py-3.5 transition-colors hover:bg-surface-0">
