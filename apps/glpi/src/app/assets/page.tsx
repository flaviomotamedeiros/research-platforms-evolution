...
import { StatTile, Skeleton, StatusChip } from '@/components/ui'
            ...
            return (
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${
                  type === tp ? 'bg-brand text-white' : 'bg-surface-1 text-ink-2 shadow-card hover:text-ink-1'
                }`}
              >
                {label}
              </button>
          ...
          {shown.map((a) => (
            <div key={a.id} className="rounded-2xl bg-surface-1 p-5 shadow-card">
              <div className="flex items-start gap-3">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-brand-light text-xl">{a.type.icon}</span>
